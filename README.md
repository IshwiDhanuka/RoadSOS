...
- `RoadSOS-Release.apk` - The compiled, production-ready Android App bundle you can install right now.
...
```bash
adb install RoadSOS-Release.apk
```
*Note for Developers:* If you are building the app from source, you **must** supply your own Google Maps API Key. 
Open `mobile/android/app/src/main/AndroidManifest.xml` and replace `PLACEHOLDER_KEY` with your actual Google Cloud API Key, otherwise the map tiles will not render (though the pins will still plot!).
...
3. Use the **Simulate Offline Mesh** toggle on the home screen.
...
### 3. The Backend
The mobile app attempts to post live payloads to `https://roadsos-api.onrender.com/api`. Ensure this endpoint is deployed and active to view live interactions, otherwise the app falls back to its offline mock data layer natively.
